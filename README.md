...
> What are the top 10 stocks by gain in the Hong Kong market today?

> Show me the latest analyst ratings and price target range for $AAPL.
...
The server starts on `http://localhost:8000`. Each MCP request must include a Longbridge OAuth access token as `Authorization: Bearer <token>`.

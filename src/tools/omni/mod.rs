...
impl Longbridge {
    ...
    #[tool(
        ...
        description = "Run a Longbridge MCP tool by name: {\"tool\": \"quote\", \"arguments\": {\"symbols\": [\"700.HK\"]}}. Or a pipeline: {\"steps\": [{\"id\": \"picks\", \"tool\": \"screener_search\", \"arguments\": {...}, \"jq\": \".data | map(.symbol)\"}, {\"id\": \"quotes\", \"tool\": \"quote\", \"arguments\": {\"symbols\": {\"$from\": \"picks\"}}}], \"return\": [\"quotes\"]} where any argument may be {\"$from\": id, \"jq\": expr}. Max 10 steps, 30 s. Write tools are two-step (dry-run then execute=confirmation_code); at most one write step per pipeline and all of its arguments must be literal values, no `$from`. Use `search` to find tools and `docs` for schemas."
    )]
    async fn execute(
        &self,
        ctx: RequestContext<RoleServer>,

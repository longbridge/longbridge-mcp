...
impl Longbridge {
    ...
    #[tool(
        ...
        annotations(read_only_hint = true, idempotent_hint = true, open_world_hint = true),
        description = "Screen stocks. market: US|HK|CN|SG. Mode A: strategy_id from screener_recommend_strategies — auto-runs saved strategy. Mode B: conditions=[\"KEY:MIN:MAX\",...], filter_ prefix auto-added, open bound = omit value (\"roe:15:\" means ROE≥15%). extra_returns=[\"key\",...] adds display-only columns not used as filters. sort_by_key=\"key\" sorts by that indicator (default: first condition). sort_order: asc|desc (default desc). Returns {total, items[]{symbol,name,indicators[]{key,value,unit}}}. IMPORTANT: indicator keys and units are platform-managed and may change. The list below is for common stable cases only — always call screener_indicators first when the key is uncertain, when results are empty, or when you need the exact unit. Commonly stable keys: pettm(P/E TTM) pb ps roe roa grossmargin netmargin netprofitgrowthrate revenuegrowthrate marketcap balance divyld currentratio debtassetratio eps."
    )]

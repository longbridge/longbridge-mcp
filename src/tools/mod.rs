...
impl Longbridge {
    ...
    #[tool(
        ...
        annotations(read_only_hint = true, idempotent_hint = true, open_world_hint = true),
        description = "Screen stocks. market: US|HK|CN|SG (Mode B required; Mode A uses strategy's market). Mode A: strategy_id from screener_recommend_strategies — auto-runs saved strategy. Mode B: conditions=[{\"key\":\"filter_KEY\",\"min\":\"10\",\"max\":\"50\",\"tech_values\":{}},...] passed directly to API. extra_returns=[\"filter_key\",...] adds display-only columns. sort_by_key: key name to sort by; sort_order: asc|desc (default desc). page: 0-based (default 0). Returns {total, items[]{symbol, name, indicators[]{key, name, value, unit}}}. Fundamental keys: filter_pettm filter_pbmrq filter_roe filter_roa filter_netmargin filter_salesgrowthyoy filter_netincomegrowthyoy filter_marketcap(亿) filter_circulating_marketcap(亿) filter_prevclose filter_prevchg(%) filter_divyld filter_la filter_epsttm filter_netincome(亿) filter_sales(亿) filter_turnover_rate filter_balance(万). Technical keys (call screener_indicators for tech_values schema): filter_macd_day/week filter_rsi_day/week filter_kdj_day/week filter_boll_day/week."
    )]
